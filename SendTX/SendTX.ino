...
void loop() {
  ...
  if (currPeriod != lastPeriod)
  {
    ...
    Number1 = Number1 + 1;
    sprintf(buff1, ":%d", Number1);
    sendSize1 = strlen(buff1);
    Number2 = radio.getAckRSSI(), DEC;
    sprintf(buff2, ":%d", Number2);
    sendSize2 = strlen(buff2);
    Number3 = radio._transmitLevel, DEC;
    sprintf(buff3, ":%d", Number3);
    sendSize3 = strlen(buff3);

    if (radio.sendWithRetry(GATEWAYID, buff1, sendSize1)) {
      radio.sendWithRetry(GATEWAYID, buff2, sendSize2);
      radio.sendWithRetry(GATEWAYID, buff3, sendSize3);
